...
expty transVar(S_table venv, S_table tenv, A_var v) {
	...
	case A_fieldVar: {
		expty ty = transVar(venv, tenv, v->u.field.var);
...
void  transDec(S_table venv, S_table tenv, A_dec d) {
	...
	case A_varDec: {
		...
		//Ty_ty ty = transTy(venv, tenv, d->u.var.typ);
		// var x: type-id = exp;
		// if (ty ==  e.ty)
		//S_endScope(venv, d->u.var.var, e);
...
expty  transExp(S_table venv, S_table tenv, A_exp a) {
	...
	case A_assignExp: {
		// lhs = rhs
		expty lhs =  transVar(venv, tenv, a->u.assign.var);
		expty rhs = transExp(venv, tenv, a->u.assign.exp);
		if (lhs.ty != rhs.ty) {
	...
	case A_forExp: {
		expty exp;
		expty lo = transExp(venv, tenv, a->u.forr.hi);
		expty hi = transExp(venv, tenv, a->u.forr.lo);
